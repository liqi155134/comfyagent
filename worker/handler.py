...
# Diagnostics go through plain print() so they land directly on RunPod's
# stdout log stream. Every line is prefixed with "worker-comfyui - " so it
# can be grep'd / filtered alongside ComfyUI's own logs.
...
s3_client = None
if BUCKET_ACCESS_KEY_ID and BUCKET_SECRET_ACCESS_KEY:
    try:
        ...
        print(f"worker-comfyui - R2 Init Failed: {e}")
...
def upload_to_r2(image_bytes, filename, job_id, user_id):
    if not s3_client or not BUCKET_NAME: return None
    try:
        # Keep the original extension on the R2 key so the object is identifiable
        # in the R2 console and any downstream tool that sniffs by suffix.
        # Falls back to .png to match ComfyUI's default SaveImage output.
        ext = os.path.splitext(filename or "")[1].lower() or ".png"
        content_type = _CONTENT_TYPE_BY_EXT.get(ext, "application/octet-stream")
        # user_id may be None if the caller forgot to send it — bucket prefix
        # "None/..." is ugly; coerce to a safe sentinel.
        safe_user_id = user_id or "anonymous"
        unique_name = f"{safe_user_id}/{job_id}/{uuid.uuid4()}{ext}"
        ...
        presigned_url = s3_client.generate_presigned_url(
            ...
            ExpiresIn=604800  # 7 days; frontend re-signs via /api/r2-sign on 403
        )

        return presigned_url

    except Exception as e:
        print(f"worker-comfyui - R2 Upload Error: {e}")
        return None
...
# WS 静默超过这么久(秒)就查一次 /history 兜底。
# 不设"主循环绝对超时":那会引入第二个必须和平台 execution timeout 手动同步的
# 旋钮,两个旋钮失配过一次就会把已完成采样、正在 VAE decode 的任务砍在终点线前。
# 空转的根因是丢事件,堵住它就够,绝对上限交给平台。
WS_SILENCE_PROBE_S = int(os.environ.get("WS_SILENCE_PROBE_S", 60))
...
def validate_input(job_input):
    ...
    if images is not None:
        # isinstance(image, dict) 必须放在最前:对非 dict 元素(如 [123]、[None])
        # 做 `"name" in image` 会抛 TypeError,冒泡到框架而不是返回友好错误。
...
def _history_settled(prompt_id):
    """查 /history 判断 prompt 是否已终结,返回 (settled, errors)。

    存在的理由:ComfyUI 的 WebSocket 是广播且**不重放**。断线窗口里推送的
    `executing:node=None` 完成事件永远补不回来,只靠 WS 等于把"丢一次事件"
    变成"空转到平台 execution timeout"——烧满计费、零产出,还报一个和真实
    原因无关的错。/history 是 ComfyUI 唯一的权威终态来源。

    查不到 / 查失败一律返回 (False, []):兜底逻辑绝不能制造假终态,
    否则会把"还在跑"误判成"已结束",把真实产物丢掉。宁可继续等,
    绝对上限交给平台的 execution timeout。
    """
    try:
        history = get_history(prompt_id)
    except Exception as e:
        print(f"worker-comfyui - History probe failed (keep waiting): {e}")
        return False, []
    ...
    if not isinstance(status, dict):
        # 老版本 ComfyUI 的 history 条目没有 status 字段,只能靠 outputs 判断
        return (True, []) if entry.get("outputs") else (False, [])

    if status.get("status_str") == "error":
        found = []
        for msg in status.get("messages", []):
            # messages 形如 [event_name, payload] 的二元组
            if (
                ...
                found.append(
                    f"Workflow execution error: Node Type: {d.get('node_type')}, "
                    f"Node ID: {d.get('node_id')}, Message: {d.get('exception_message')}"
                )
        if not found:
            found.append(
                "Workflow execution failed (history reports error, no detail available)"
            )
        return True, found
    ...
    try:
        ...
        while True:
            try:
                ...
                # 探测点 ①:WS 长时间静默。可能是任务真的在慢慢跑,也可能是
                # 完成事件已经在某次断连窗口里丢掉了 —— 后者只有 /history 能分辨。
                silent_for = time.time() - last_msg_at
                if silent_for >= WS_SILENCE_PROBE_S:
                    settled, hist_errors = _history_settled(prompt_id)
                    if settled:
                        print(
                            f"worker-comfyui - Settled via history probe after "
                            f"{silent_for:.0f}s of websocket silence"
                        )
                        if hist_errors:
                            ...
                            execution_done = True
                        break
                    # 未终结:重置计时,避免每次 recv 超时都打一次 /history
                    last_msg_at = time.time()
                ...
                try:
                    ...
                    # 探测点 ②:重连成功。断线期间的广播不会重放,要等的那条
                    # `executing:node=None` 很可能已经播过了 —— 原来的
                    # "resuming message listening" 是在等一条永远不会再来的消息。
                    settled, hist_errors = _history_settled(prompt_id)
                    if settled:
                        print(
                            "worker-comfyui - Prompt already settled during the "
                            "disconnect window (detected via history probe)"
                        )
                        if hist_errors:
                            errors.extend(hist_errors)
                        else:
